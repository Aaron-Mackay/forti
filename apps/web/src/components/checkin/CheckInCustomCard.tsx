 ...
 */
export default function CheckInCustomCard({ card, gridColumn, responses, onChange, signalEnabled = false }: Props) {
  ...
  if (signalEnabled) {
    return (
      <Box sx={{ gridColumn, minWidth: 0 }}>
        <SignalSectionCard eyebrow={card.title || undefined}>{fields}</SignalSectionCard>
      </Box>
    );
  }
