/**
 * Shared seeding logic for the TestUser E2E test account.
 *
 * Used by both seed.ts (full reset, fixed past date for CI) and any other
 * callers that need to populate a TestUser-shaped account.
 *
 * Callers are responsible for ensuring the user and exercises already exist
 * in the DB before calling seedTestUserData.
 */
...
const PLAN_TEMPLATES = [
  {
    name:        "TestUser's Plan 1",
    description: 'Training block 1 for TestUser',
    weekCount:   2,
    workoutTemplates: [
  ...
  {
    name:        "TestUser's Plan 2",
    description: 'Training block 2 for TestUser',
    weekCount:   3,
    workoutTemplates: [
...
/**
 * Resets and recreates all of the test user's data relative to `today`.
 * Safe to call after a full TRUNCATE (deleteMany is a no-op) or against
 * an existing DB (deleteMany cleans up first).
 */
export async function seedTestUserData(user: { id: string }, today: Date): Promise<void> {
